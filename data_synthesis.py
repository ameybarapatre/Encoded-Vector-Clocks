...
def synthesize(n , sthreshold = 0.5, rthreshold = 0.5 , max =32):
    ...
    while mp.log(np.max(evc)) /mp.log(2) <= max*n :
        ...
        if np.random.random_sample() > sthreshold :
            ...
            if mp.log(np.max(evc)) / mp.log(2) > max* n :
               break
    ...
    np.savetxt("./Experiment 1A/" + str(n)+"-0.4.out", np.array(dataout))
...
if __name__ == '__main__':
    #for i in range(2,100,10):
      #print(synthesize(n=i ,max = 64))
    #print(synthesize(10 , rthreshold= .4, sthreshold=.4,max = 64))

    print(exp2_synthesize(50, max=32))